...
summar <- function(logit) {
  ...
  summary <- summary %>%
    tibble::rownames_to_column(var = "Predicator") %>%
...
ord_summary <- function(ord_logit, confint = TRUE) {
  ...
  if (confint) {
    coef_table <- as.data.frame.matrix(cbind(
    ...
    coef_table <- as.data.frame.matrix(cbind(
      Odds_Ratio = exp(coefs[, 1, drop = FALSE]),
      SE = coefs[, 2],
      p_vals = coefs[, 3] %>% abs() %>% pnorm(lower.tail = FALSE) * 2
    ))
  }
...
plot_summary <- function(summary, logit = NULL, title = "Regression Summary", digits = 2, add_ref = TRUE) {
  ...
  row.names(summary) <- lapply(row.names(summary), FUN = function(x) {
    ifelse(x %in% names(factor_label_dict),
...
summar <- function(model, truncate_p = TRUE, p_het = NULL, CI = TRUE, signif = FALSE, n = FALSE, latex = TRUE, ref = TRUE, sep = F) {
  ...
  if (ref) {
    refs <- lapply(model$xlevels, function(x) {
      x[1]
    }) %>%
      {
        .[. != "0"]
  ...
  if (sep) {
    summary <- summary %>%
      tidyr::separate(CI, c("upper", "lower"), "-") %>%
...
add_significance_annotation <- function(p, label, pos, start, end, size_text = 10, size_line = 1) {
  ...
  p +
    ...
    annotate("segment", x = pos - 0.2, xend = pos + 0.008, y = start, yend = start, size = size_line) +
    annotate("segment", x = pos - 0.2, xend = pos + 0.008, y = end, yend = end, size = size_line)
}
...
model_growth_rate_class_fun <- function(df_for_glm) {
  purrr::cross_df(list(
    ...
    pmap_dfr(function(Gene_Class, comparison) {
      ...
      fit <- glm(formula = growth_rate ~ Cohort + AGE + SEX + RACE, data = df)
    ...
    filter(term %in% c("UntreatedVSCarboplatin", "UntreatedVSPARPi", "CarboplatinVSPARPi"))
}

make_mouse_cell_plot <- function(
  data, cell_col, title, 
  y_label = "",
  y_limits = c(0, 2.5), 
  x_limits = as.character(0:13), 
  x_breaks = c(0, 4, 8, 12)
) {
  plot <- data %>%
    group_by(Cohort, Week) %>%
    summarise(
      mean_value = mean(.data[[cell_col]], na.rm = TRUE),
      sd_value = sd(.data[[cell_col]], na.rm = TRUE),
      sem_value = sd(.data[[cell_col]], na.rm = TRUE) / sqrt(n()),
      .groups = "drop"
    ) %>%
    ggplot(
      aes(
    ...
    scale_y_continuous(limits = y_limits, labels = scales::label_number(accuracy = 0.1)) +
    theme(
    ...
    scale_x_discrete(limits = x_limits, expand = c(0, 0.5), breaks = x_breaks) +
